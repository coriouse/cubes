...
public class PuzzleResolver {
	...
	public final static int TRANSPOSITION_COUNTER = 2;
	public final static int ROTATE_COUNTER = 4;

	private Map<Integer, Square> happyElements;
	private List<Integer[]> listPositions = new ArrayList<>();
	private String solution;

	public PuzzleResolver(Map<Integer, Square> happyElements, String solution) {
	...
	private void searchCubes(Integer[] positions) {
		...
		while (trnspSide < TRANSPOSITION_COUNTER) {
			int rotateSide = 0;
			// search sides by SIDE
			while (rotateSide < ROTATE_COUNTER) {
				int trnspRight = 0;
				// transposition of RIGHT
				while (trnspRight < TRANSPOSITION_COUNTER) {
					int rotateRight = 0;
					// search sides by RIGHT
					while (rotateRight < ROTATE_COUNTER) {
						int trnspRightNext = 0;
						// transposition of RIGHT_NEXT
						while (trnspRightNext < TRANSPOSITION_COUNTER) {
							int rotateRightNext = 0;
							// search sides by RIGHT_NEXT
							while (rotateRightNext < ROTATE_COUNTER) {
								int trnspRightNextNext = 0;
								// transposition of RIGHT_NEXT_NEXT
								while (trnspRightNextNext < TRANSPOSITION_COUNTER) {
									int rotateRightNextNext = 0;
									// search sides by RIGHT_NEXT_NEXT
									while (rotateRightNextNext < ROTATE_COUNTER) {
										int trnspTop = 0;
										// transposition of TOP
										while (trnspTop < TRANSPOSITION_COUNTER) {
											int rotateTop = 0;
											// search sides by TOP
											while (rotateTop < ROTATE_COUNTER) {
												int trnspBottom = 0;
												// transposition of BOTTOM
												while (trnspBottom < TRANSPOSITION_COUNTER) {
													int rotateBottom = 0;
													// search sides by BOTTOM
													while (rotateBottom < ROTATE_COUNTER) {
														...
														// search
														// RIGHT_NEXT_NEXT
														if (ResolverUtils.isEqualSide(rightNext.getRight(),
																rightNextNext.getLeft())
																&& ResolverUtils.isEqualSide(side.getLeft(),
																		rightNextNext.getRight())) {
														...
														if (ResolverUtils.isEqualSide(side.getTop(), top.getBottom())
																...
																&& ResolverUtils.isEqualSideRevers(rightNext.getTop(),
																		top.getTop())
																&& ResolverUtils.isEqualSideRevers(rightNextNext.getTop(),
																		top.getLeft())
																&& (side.getTop()[4] + top.getBottom()[4]
																		+ right.getLeft()[4]) == 1
																&& (right.getTop()[4] + top.getRight()[4]
																		+ rightNext.getLeft()[4]) == 1
																&& (rightNext.getTop()[4] + top.getTop()[0]
																		+ rightNextNext.getLeft()[4]) == 1
																&& (rightNextNext.getRight()[4] + top.getLeft()[0]
																		+ side.getTop()[0]) == 1) {
														...
														if (ResolverUtils.isEqualSide(side.getBottom(), bottom.getTop())
																&& ResolverUtils.isEqualSideRevers(right.getBottom(),
																		bottom.getRight())
																&& ResolverUtils.isEqualSideRevers(rightNext.getBottom(),
																		bottom.getBottom())
																&& ResolverUtils.isEqualSide(rightNextNext.getBottom(),
																		bottom.getLeft())
																&& (side.getBottom()[4] + right.getLeft()[0]
																		+ bottom.getTop()[4]) == 1
																&& (right.getBottom()[4] + rightNext.getLeft()[0]
																		+ bottom.getRight()[0]) == 1
																&& (rightNext.getBottom()[4]
																		+ rightNextNext.getLeft()[0]
																		+ bottom.getBottom()[0]) == 1
																&& (rightNextNext.getBottom()[4] + side.getLeft()[0]
																		+ bottom.getTop()[0]) == 1) {
