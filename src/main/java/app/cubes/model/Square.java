...
public class Square {
	...
	private static final Map<String, Square> BIND_SIDES = new HashMap<String, Square>(4);
	private int[][] square = new int[SIDE_SIZE][SIDE_SIZE];
	private int[][] original = new int[SIDE_SIZE][SIDE_SIZE];
	...
	public void transposition() {
		...
		trans = new int[SIDE_SIZE][SIDE_SIZE];
		originalTrans = new int[SIDE_SIZE][SIDE_SIZE];
	...
	public void rotate() {
		...
		rotated = new int[SIDE_SIZE][SIDE_SIZE];
		rotateOriginal = new int[SIDE_SIZE][SIDE_SIZE];
		int r = 0, c = 0;
	...
	public String toString() {
		return "Square [Number] =" + getNumber() + " [left] =" + Arrays.toString(this.getLeft()) + ",[right] = "
				+ Arrays.toString(this.getRight()) + ", [bottom] =" + Arrays.toString(this.getBottom()) + ", [top]="
				+ Arrays.toString(this.getTop());
	}
	...
	public void printUnfolded(String solution) {
		...
		if (top != null) {
			for (int i = 0; i < top[0].length; i++) {
				for (int j = 0; j < top.length; j++) {
					sb.append(EMPTY).append(getChar(top[i][j]));
				}
				sb.append("\n");
			}
		}
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				sb.append(EMPTY).append(getChar(original[i][j]));
			}
			sb.append(EMPTY).append(SEPARATOR);
			if (right != null) {
				for (int j = 0; j < size; j++) {
					sb.append(EMPTY).append(getChar(right[i][j]));
				}
			} else {
				for (int j = 0; j < size; j++) {
					sb.append(EMPTY).append(SPACE);
				}
			}
			sb.append(EMPTY).append(SEPARATOR);
			if (mirrorThis != null) {
				for (int j = 0; j < size; j++) {
					sb.append(EMPTY).append(getChar(mirrorThis[i][j]));
				}
			} else {
				for (int j = 0; j < size; j++) {
					sb.append(EMPTY).append(SPACE);
			...
			if (left != null) {
				for (int j = 0; j < size; j++) {
					sb.append(EMPTY).append(getChar(left[i][j]));
				}
			} else {
				for (int j = 0; j < size; j++) {
					sb.append(EMPTY).append(SPACE);
		...
		if (bottom != null) {
			for (int i = 0; i < bottom[0].length; i++) {
				for (int j = 0; j < bottom.length; j++) {
					sb.append(EMPTY).append(getChar(bottom[i][j]));
				}
				sb.append("\n");
			}
		}
